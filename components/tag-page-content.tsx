...
export function TagPageContent({
  ...
  const categoryNames = useMemo(
    () => normalizedCategories.map((category) => category.name),
  ...
  const selectedTag = useMemo(() => {
    ...
    const match = normalizedTags.find((tag) => tag.slug === initialTagSlug);
    return match?.slug ?? null;
  }, [initialTagSlug, normalizedTags]);
  ...
  useEffect(() => {
    setSelectedCategories((previous) =>
      previous.filter((category) => categoryNames.includes(category)),
    );
  }, [categoryNames]);
  ...
  const filteredTools = useMemo(() => {
    ...
    return tools
      .filter((tool) => {
        if (selectedTag && !tool.tags.some((tag) => tag.slug === selectedTag)) {
          return false;
        }
  ...
  }, [filterPreset, searchTerm, selectedCategories, selectedTag, sortOption, tools]);
        ...
        <main className="flex flex-1 flex-col items-center">
          <section className="flex w-full max-w-4xl flex-col items-center gap-4 text-center">
            ...
            >
              ...
              Explore by tags and surface your next favourite tool
              <span className="text-base">→</span>
            </Link>
            <h1 className="max-w-3xl text-4xl font-semibold tracking-tight text-[#17171c] sm:text-[3.15rem] sm:leading-[1.08]">
              Discover tools by <span className="text-[#ff7d68]">tag</span>
            </h1>
            <p className="text-base text-[#63636a] sm:text-lg">
              Jump into curated collections by topic, workflow, and trend. Pick a tag to see the tools that makers love right now.
            </p>
          </section>
            ...
            <div className="mt-16 flex flex-col items-center gap-4 rounded-2xl border border-dashed border-[#ffd1c6] bg-[#fff7f4] px-10 py-12 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="max-w-md text-sm text-[#7f7f88]">
                  Tweak the filters or clear them to browse the full collection of tools.
                </p>
              </div>
              <Button
                type="button"
